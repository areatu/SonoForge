...
1. В worksheet выберите строку **A4C ED** или **A4C ES** (или A2C) — задаётся контекст фазы.
2. На соответствующем кадре нажмите **`I`** (Auto Segment в SystemBar) — ONNX строит open-arc контур эндокарда (`source="ai"`).
3. Нажмите **`R`** для gradient refine (active contour с Lamé fallback на Laplacian smooth).
4. Перетащите сплайн-узлы для коррекции; панель результатов пересчитает объёмы по Симпсону.

Если модель недоступна или инференс превышает таймаут, в статус-баре появится подсказка использовать ручной контур (`C` / worksheet Manual Simpson).
