...
def _write_dicom_cine(tmp_path, n_frames: int = 10):
    """Write an uncompressed n-frame 32x32 DICOM clip and return its path."""
    ...
    ds.PixelData = np.random.randint(0, 256, (n_frames, 32, 32), dtype=np.uint8).tobytes()
    path = tmp_path / f"cine_{n_frames}.dcm"
...
class TestSpeckleTrackingWorkerRun:
    ...
    def test_run_decodes_full_cine_from_source_when_frames_none(
        ...
        mock_assign.return_value = list(mock_kernels.return_value)
        ...
        patch_targets = {
            "preprocess_echo_frame": patches["preprocess_echo_frame"],
            "build_zone_mask": patches["build_zone_mask"],
            "track_cine_incremental": patches["track_cine_incremental"],
            "extract_trajectories": patches["extract_trajectories"],
            "interpolate_invalid_kernels": patches["interpolate_invalid_kernels"],
            "smooth_trajectories": patches["smooth_trajectories"],
            "apply_motion_model": patches["apply_motion_model"],
            "compute_weighted_longitudinal_strain_gl": patches["compute_weighted_longitudinal_strain_gl"],
            "compute_weighted_radial_strain_gl": patches["compute_weighted_radial_strain_gl"],
            "compute_strain_rate": patches["compute_strain_rate"],
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch(
                    "echo_personal_tool.application.workers.speckle_worker.track_cine_sequential",
                    MagicMock(return_value=[MagicMock(displacements=np.zeros((6, n_kernels, 2)))]),
                )
            )
            ...
            # The worker decoded the whole clip from disk on its own thread.
