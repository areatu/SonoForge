...
def test_detect_grid_lines_basic():
    frame = _make_frame_with_grid_lines(
        height=200, width=300, grid_line_ys=[50, 100, 150]
    )
    lines = detect_doppler_grid_lines(
        frame, x0=0, y0=0, width=300, height=200
    )
...
def test_detect_grid_lines_with_roi_offset():
    frame = _make_frame_with_grid_lines(
        height=300, width=400, grid_line_ys=[100, 150, 200]
    )
    lines = detect_doppler_grid_lines(
        frame, x0=50, y0=80, width=200, height=120
    )
...
def test_detect_grid_lines_single_line():
    frame = _make_frame_with_grid_lines(
        height=200, width=300, grid_line_ys=[100]
    )
    lines = detect_doppler_grid_lines(
        frame, x0=0, y0=0, width=300, height=200
    )
...
def test_detect_grid_lines_small_roi():
    ...
    lines = detect_doppler_grid_lines(
        frame, x0=100, y0=40, width=50, height=30
    )
...
def test_detect_grid_lines_out_of_bounds_roi():
    ...
    lines = detect_doppler_grid_lines(
        frame, x0=250, y0=180, width=100, height=50
    )
    assert isinstance(lines, list)
