...
def test_log_compress_range() -> None:
    col = np.array([0, 128, 255], dtype=np.uint8)
    result = log_compress(col)
    assert result.dtype == np.float32
    assert result[0] == 0.0
    assert result[2] > result[1] > 0.0


def test_log_compress_preserves_order() -> None:
...
def test_spatial_smooth_reduces_jaggedness() -> None:
    ...
    col[45:55] = 255.0
    smoothed = spatial_smooth(col, sigma=1.5)
    # Edges should be softened
    assert smoothed[44] > 0.0
    assert smoothed[55] > 0.0
    # Peak should be slightly reduced
    assert smoothed[50] < 255.0
