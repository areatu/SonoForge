...
|---|---|---|---|---|
| замыкание > гейта, 40 дБ | 4.8 | 1.06 мм | 4.55 мм | 0.19 / 0.47 |
| NCC < 0.3, 40 дБ | **0.0** | 1.22 мм | — | 0.00 / 0.00 |
| NCC < 0.7, 40 дБ | 3.6 | 1.11 мм | 4.47 мм | 0.21 / 0.40 |
| замыкание > гейта, 20 дБ | 6.1 | 3.35 мм | 4.23 мм | 0.55 / 0.10 |
| NCC < 0.3, 20 дБ | **0.0** | 3.43 мм | — | 0.00 / 0.00 |
| NCC < 0.7, 20 дБ | 3.8 | 3.43 мм | 3.65 мм | 0.38 / 0.04 |
| замыкание > гейта, 10 дБ | 76.3 | 19.63 мм | 34.33 мм | 1.00 / 0.76 |
| NCC < 0.3, 10 дБ | **0.0** | 33.69 мм | — | 0.00 / 0.00 |
| NCC < 0.7, 10 дБ | 11.1 | 33.55 мм | 35.39 мм | 1.00 / 0.11 |
...
2. **Замыкание разделяет на всех уровнях** и особенно там, где это важно:
   19.63 мм против 34.33 мм на 10 дБ при recall 0.76.

Харнесс воспроизводит долю QC до десятых (4.8 / 6.1 / 80.6 %), то есть меряет
тот же флаг, который стоит в отчёте, а не свою его версию.

**Условия воспроизведения** (числа выше зависят от них и воспроизводятся
буквально): python 3.11, numpy **1.26.4** (`pyproject.toml` держит `numpy<2.0` —
под numpy 2.x сглаживание траекторий даёт другие значения: например, ошибка
эндокарда p95 на 20 дБ читалась как 4.55 мм вместо 3.38 мм), scipy 1.17,
opencv 4.11, фантом `quick` (200×200, 20 кадров, endo 10.0 мм).
...
`bench/ste_phantom.py`: 20 вариантов, 16 KPI-провалов — **все на строках
`invalid`, кроме одной**: `zero_snr20` (`review`, «движения нет вообще», поэтому
segment RMS — чистый шум отслеживания) даёт 2.51 п.п. против порога 2.5, то есть
стоит ровно на пороге. На строках со статусом `valid` провалов нет. Клинически
...
Разложение сторон стенок по изображению (см. §6) перераспределило узлы на
границах третей: per-segment RMS сместился на десятые доли (uniform 20 дБ
1.68 → 1.74 п.п., zero 20 дБ 2.40 → 2.51 п.п.; long_axis 1.10 → 1.06). GLS и
TTP не изменились ни на одной строке.
...
| «сегменты — по анатомии на КД; равная длина на КД; авто-разметку можно править; выбирать вид, инверсию, флип» | **закрыто** | сегменты по дуге КД (`assign_segments_from_arc`, равная доля дуги от аннулуса); сторона стенки — по изображению (половина дуги с меньшим `x` несёт первую стенку вида), поэтому разворот обводки не переименовывает сегменты; вид A4C/A2C/A3C и «Зеркальное отображение» (`SpeckleConfig.segment_flip`) выбираются, объявление — `segment_sides` в JSON/CSV и метке strain-окна. Отдельный интерактивный редактор границ сегментов (сверх правки самого контура) — пункт UX-бэклога, не требование консенсуса |
...
| «информировать о мерах регуляризации; ограничивать её минимумом; давать контроль» | **закрыто** | `StrainResult.regularization` + CSV `# Regularization`; объявляется **фактически применённое** окно Савицкого–Голея (нечётное, обрезанное по длине окна анализа, либо «фильтр не применялся» — `effective_curve_smoothing_window`); окно задаётся пользователем (1–31 кадр) в настройках трекинга, а слайдер в окне деформации переглаживает траектории пост-фактум |
...
| «объявлять источник обводки, если контур не размечен вручную» (следствие требований к сопоставимости) | **закрыто** | `StrainReport.contour_source`: значение `draft` печатает в протоколе строку «Контур КД: черновик (не размечен вручную)» — полуавтоматический контур не выдаётся за ручную разметку |
...
Итого: все определения, элементы управления и объявления, которые консенсус
требует для сопоставимости чисел, закрыты. Единственное принципиальное
ограничение — через-плоскостное движение (2D), оно не «исправляется».
Из UX-бэклога остаётся один пункт, не требуемый консенсусом: интерактивный
редактор границ сегментов (сверх правки самого контура).
...
`tests/unit/test_ste_protocol_report.py` — 29 тестов на выход чисел за пределы
strain-окна (план §5.3 п.6): отчётная проекция study, текст секции и PDF
(включая глифы меток ●/⚠ в шрифте отчёта), накопление видов в контроллере,
перенимание study окном и **паритет «числа UI == числа отчёта»** — таблица окна
и форматтер получают одни и те же `StrainResult`, поэтому расхождение любой
стороны роняет тест (проверено мутацией определения `GLS_AV`).
