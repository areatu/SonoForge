...
> **Версия:** rev.6 (rev.5 + сессия 3: §5.3 п.6 закрыт — GLS в протоколе и отчёте, фаза 3 закрыта полностью)
> **Дата:** 2026-09-11
> **Статус:** спека действует; реализация — ветка `arena/01a08cb0-sonoforge`, PR #74. Фазы 0–3 закрыты,
> фаза 4 — частично, фаза 5 — частично (построчный статус §5.2, очередь работ §5.3, окружение §5.4).
> Консенсус EACVI/ASE (Voigt 2015) закрыт полностью, кроме принципиального ограничения 2D.
> Решения приняты (§11.1), открытые вопросы — §11.2 (два блокера сверки/merge, разработку не блокируют).
...
> **Реальные данные (с 2026-09-11, с 2026-09-17 — в закрытом репозитории `areatu/Sonoforge_data`):** `data/dicom/For_pero` (19 клипов, Git LFS) → CI-воркфлоу `.github/workflows/ste-fixtures.yml` выгружает компактную производную в
> `tests/fixtures/for_pero` (кадры + обезличенные заголовки + индекс). Вендорские числа по этим клипам —
> `docs/STE_VENDOR_REFERENCE.md`; чего не хватает для сверки — §11.2 п.1.
>
> **rev.1 → rev.2, главное:**
...
**rev.4 → rev.5, главное (2026-09-11):**
> 1. §5.2 приведено к факту: закрыты все шесть вопросов клинического разбора (эпикард-дуга, потеря
>    видимости стенки, несовпадение определений, доверие к слежению), полностью закрыт EACVI/ASE
>    (стороны стенок по изображению + зеркальное отображение, контроль окна сглаживания с честным
>    раскрытием применённого значения), фаза 4 расширена, фаза 5 частично.
> 2. Реальные клипы (`tests/fixtures/for_pero`, 19 клипов, CI-экспорт) и вендорские числа
>    (`docs/STE_VENDOR_REFERENCE.md`) теперь **в репозитории** — см. §7.3. Google Drive по-прежнему
>    недоступен, но больше не нужен.
> 3. Подробный отчёт о валидации и соответствии вынесен в `docs/STE_TRACKING_VERIFICATION.md`
>    (фантом §3/§5, ворота KPI, матрица консенсуса §6, ограничения §4).
> 4. Добавлены §0 «Сводка для передачи: что сделано и что осталось» (один экран — итог сессии и очередь
>    на следующий чат), §5.3 «Осталось сделать» (упорядоченная очередь с критериями приёмки и оценками)
>    и §5.4 «Как продолжить» (окружение, тестовый свип, бенчмарки, артефакты).
> 5. Исправлены устаревшие ссылки «§8, многошкальное совмещение» → «§5.3, низкий SNR» (§8 теперь про
...
**Ветка** `arena/01a09219-sonoforge` (продолжение `arena/01a09076-sonoforge`, PR #75).
**Сессия 2026-09-11 (третья):** закрыт §5.3 п.6 — GLS уходит в протокол исследования и в отчёт/PDF,
фаза 3 закрыта полностью. PR **#74 закрыт** как устаревший: все его 37 коммитов — предки ветки PR #75
(проверено `git merge-base --is-ancestor`), ничего не потеряно.

**Предыдущая запись (сессия 2)** — ветка `arena/01a09076-sonoforge`, PR **#75**.
**Head — верхний коммит ветки** (`git log -1`); на момент этой записи — `7978f80`, дерево чистое,
**все проверки PR #75 зелёные** (14 pass, 1 skip): `test (ubuntu-latest, 3.11)` 38 мин 36 с,
`Test coverage` 34 мин 9 с; на последнем кодовом коммите `1683ab9` — ubuntu 33 мин 3 с / coverage 35 мин 28 с
(macOS 4 мин 55 с, Windows 11 мин 43 с, `lint` 9 с, CodeQL, сборки, бенчмарки — зелёные в обоих прогонах).
Коммиты этой сессии: `3659d4a` (красный CI), `52a6366` (подписи сегментов §5.3 п.4), `b38c6bc` (док —
зелёный CI), `1683ab9` (ориентация иллюстраций, кодовый), `7978f80` и последующие — только документация.
PR #74 оставался открытым из ветки позапрошлой сессии; закрыт 2026-09-11 в третьей сессии.

**Что взять в новую сессию:** очередь §5.3 — P1 п.2 (обзорный экран трёх видов, 3–5 дн) и п.3 (авто-черновик
контура КД, нужны веса ONNX); P0 п.1 (сверка с вендором) — пользователь разрешил считать на
**полуавтоматическом контуре с явной пометкой «не gold»** (§11.2 п.1, альтернативный вариант), инфраструктура
для пометки уже есть: `StrainReport.contour_source='draft'` печатает строку «Контур КД: черновик».
Окружение и проверки — §5.4, включая рецепт восстановления локального git после сброса песочницы.
...
### Сделано (сессия 3, 2026-09-11)

* **GLS в протоколе исследования и в отчёте (§5.3 п.6, фаза 3 закрыта).** Секция «Продольная деформация
  (STE)» в тексте отчёта и в PDF: GLS по каждому виду с QC-меткой ●/⚠/■, `GLS AV` только по валидным
  видам, метод (Лагранж, пик, эндокард, AHA-18), источник закрытия АК по видам, число неизмеренных
  сегментов, объявление черновика контура.
* **Одно накопление видов вместо двух.** `StrainReport` — отчётная проекция `StrainStudy`
  (`strain_report_from_study()`); хранение на исследование (`StudyMeasurementSessionStore.set_strain()`,
  `MeasurementSnapshot.strain`); накопление в `AppController._record_strain_result()` (повторный расчёт
  вида заменяет его); `StrainWindow.set_study()` перенимает то же накопление, поэтому закрытие окна
  больше не теряет per-view GLS и `GLS_AV`.
* **Честность отчёта:** вид без конечного GLS не печатается вовсе; отклонённые виды печатаются с меткой,
  но не входят в среднее; при отсутствии валидных видов пишется «ни один вид не прошёл контроль
  качества»; метод хранится как факт и переводится при смене языка, а не замораживается.
* **Тесты:** `tests/unit/test_ste_protocol_report.py` (29), включая паритет «числа UI == числа отчёта»
  (мутация определения `GLS_AV` роняет тест) и проверку PDF вместе с глифами меток в шрифте отчёта.
* **PR #74 закрыт** как устаревший — его коммиты являются предками ветки PR #75.

### Сделано (сессия 2)

* **Определение strain приведено к клиническому**: Lagrange `(L−L₀)/L₀` вместо Green–Lagrange (F5);
  одно определение в worker, UI и экспорте, тест равенства чисел.
* **Валидация фантомом**: `tests/fixtures/ste_phantom.py` + `bench/ste_phantom.py` — 20 вариантов, точная
  кинематика; закрыты находки F1–F12 (§7.5), KPI-ворота в тестах затянуты с 7.0 до 1.0 п.п., инварианты
  жёсткого движения; числа воспроизводимы на `numpy==1.26.4` (`docs/STE_TRACKING_VERIFICATION.md` §3, §5).
* **Честный QC вместо «качества 95 %»**: `quality.py` (`valid/review/invalid` + причины + confidence),
  кросс-чек глобальной кривой и сегментного среднего, знаковая когерентность, покрытие/физиологичность,
  круговой проход Q6 (`closure_*`, ворота 0.10/0.25/0.30), видимость стенки Q4b, шум против сокращения F6;
  метки недостоверного слежения ✕/○ на изображении (решение раунда 3).
* **Анатомическая геометрия**: открытая дуга annulus→apex→annulus (хорда МК исключена), 18-сегментная
  AHA-карта по дуге ED, `assign_aha_segments`; **сторона стенки определяется изображением**, поэтому
  разворот обводки не переименовывает сегменты, а зеркальное отображение выбирается и объявляется.
* **Фаза 4 (частично)**: мишень 18 сегментов в раскладке GE с палитрой GE/EchoPAC, карта TTP, «нет данных»
  вместо выдуманных секторов; сводная таблица как у вендора (GLS, ESS, TTP, PSI, дрейф, per-view A4C/A2C/DAO,
  `GLS_AV`, ФВ/КДО/КСО, АвтоЗАК, ЧСС) + **метка статуса вида ●/⚠/■**; настройки (окно сглаживания 1–31 кадр,
  зеркальное отображение, толщина стенки, компенсации), QC-чекбоксы сегментов с пересчётом GLS, экспорт
  CSV/JSON/PNG.
* **Реальные данные и вендорские эталоны**: клипы `For_pero` (19 шт., LFS, закрытый репозиторий
  `areatu/Sonoforge_data`) → CI-воркфлоу
  `.github/workflows/ste-fixtures.yml` выгружает производную в `tests/fixtures/for_pero` (68 МБ, кадры +
  обезличенные заголовки + индекс); `docs/STE_VENDOR_REFERENCE.md` — вендорские GLS по видам и конвенции;
  контракт `tests/unit/test_ste_real_clips.py`.
* **Консенсус EACVI/ASE (Voigt 2015) закрыт полностью, кроме принципиального ограничения 2D** — матрица
...
* **Подписи сегментов у стенок** (вторая сессия): `presentation/segment_labels.py` — короткие
  вендорские имена («БазПерг/СрБок») для cine и панели, полные AHA-имена для списков; 18 ключей ru/en
...
5. ~~**Подписи сегментов/стенок на cine** (P1)~~ — **сделано 2026-09-11**: короткие имена у стенок
   (как «БазПерг/СрБок» у вендора) на cine и в strain-окне, якорь снаружи стенки, ru/en на все 18 сегментов.
6. ~~**GLS в протокол исследования и отчёт** (P1)~~ — **сделано 2026-09-11**: секция STE в отчёте и PDF
   (GLS по видам с QC-метками, `GLS AV` только по валидным видам, метод, источник АК, черновик контура
   объявляется), одно накопление видов на контроллере, паритет чисел UI↔отчёт под тестом. Фаза 3 закрыта.
7. **Низкий SNR: многошкальное совмещение** (P2) — 10 дБ сейчас честно `invalid`.
8. **Редактор границ сегментов, headless CLI, остаток фазы 4 UI** (P2) и расширения LA/RV, multi-beat (P3).
9. **Решение по приватности фикстур** — в кадры впаяны ФИО: маскировать в экспортёре или не коммитить
   бандл в main (тесты тогда скипаются). До решения PR в main не мержим.

**Как продолжить** — §5.4: окружение с нуля, обязательные `ruff check` + `ruff format --check`, список 23
STE-файлов (27 + `test_segment_labels.py`), бенчмарки, где лежат документы и данные.
...
7. **Ориентация/флип:** каждой паре стенок вида сопоставлены *анатомические* имена только при известной
   ...
   *Сделано:* автоопределение — по изображению (половина дуги с меньшим `x` несёт первую стенку вида),
   поэтому разворот обводки имена не меняет; ручное переключение — «Зеркальное отображение» в настройках
   трекинга (`SpeckleConfig.segment_flip`), объявление — `segment_sides` в JSON/CSV и метке strain-окна.
   Интерактивный редактор *границ сегментов* (в отличие от правки контура) остаётся пунктом UX-бэклога;
   требованием Task Force он не является.

### 3.4 Формулы (основные)

* Узловая (локальная) деформация — **клиническое Lagrange-определение** (Voigt 2015):
  `ε_i(t) = (L_i(t) − L_i(0)) / L_i(0) · 100 [%]`, где `L_i` — длина 3-узловой поддуги `x_{i−1}…x_{i+1}`
  через `x_i` (реализация: `lagrangian_strain_pct()`, `compute_node_longitudinal_curves`).
  *Историческая справка:* до 2026-09-11 считался Green–Lagrange `0.5·((L/L₀)² − 1)`, который занижает
  |ε| примерно на 2 п.п. при −20 % (находка F5, §7.5); оставлен только в тестах как контроль формулы.
* Кривая сегмента: `ε_seg(t) = Σ_i w_i·ε_i(t) / Σ_i w_i` (усреднение **в один момент времени**; веса — по ED).
  ESS/пик сегмента читаются **из кривой сегмента**, а не из пиков узлов.
* Глобальная кривая вида: (A) по всей линии `ε_gl(t) = (L_total(t) − L_total(0)) / L_total(0) · 100`, где
  `L_total` — длина открытой дуги; (B) `Σ_seg w_seg·ε_seg(t) / Σ_seg w_seg`. Основной результат — (A);
...
Последнее обновление: 2026-09-11, ветка `arena/01a08cb0-sonoforge`, PR #74 (MERGEABLE).

| Фаза | Статус | Что сделано / что осталось |
|---|---|---|
| 0. Честность и диагностика | **сделано** | Единое определение strain (`compute_node_longitudinal_curves`, `aggregate_segment_curves`, `global_curve_from_node_curves`) — клинический Lagrange `(L−L₀)/L₀`; `domain/services/quality.py` со статусами `valid/review/invalid`, причинами (i18n) и confidence; ре-интерполяция кадров удалена (`qc_interpolated_fraction`); `StrainResult` несёт `segment_curves`/`node_curves`, UI рисует кривые модели; мета-строка показывает статус + NCC-достоверность + причины + метки недостоверных узлов над изображением. **Осталось (не критично):** диагностический прогон `IM_0059` — клип в песочницу не передан, роль реальных данных выполняют 19 клипов For_pero. |
| 1. Геометрия ЛЖ и сегменты | **сделано** | `resample_open_arc()` + `resample_along_arc()` (открытая дуга, хорда исключена), arc-aware сглаживание/кламп; `domain/services/segment_map.py` — 18-сегментная AHA-карта по длине дуги и виду; `assign_aha_segments(..., view=, flip=)` — все слои узла делят его сегмент; **сторона стенки определяется изображением** (половина дуги с меньшим `x` несёт первую стенку вида), поэтому разворот обводки не переименовывает сегменты; зеркальное отображение выбирается в настройках и объявляется (`segment_sides` в JSON/CSV/метке). Эпикард — открытая дуга с landmark-узлами (Q1). **Осталось:** подписи сегментов/уровней на cine (§5.3 п.4), интерактивный редактор границ сегментов — пункт UX-бэклога (§5.3 п.7); выделение `material_line.py` в отдельный модуль не требуется (логика в `strain_computation`). |
| 2. Ядро расчёта | **сделано** | `strain_metrics.py`: окно полного цикла (RR → HR → нормальное отношение систолы), `detect_avc_frame()` с источником (manual → ЭКГ → площадь Симпсона → пик strain → ES), GLS = пик глобальной кривой, ESS, TTP, PSI, измеренный дрейф; среднее пиков сегментов — только кросс-чек. Закрыты находки фантома F1–F12 (§7.5): окно поиска, стационарный кламп, жёсткое движение, честный QC, определение strain, шум/сокращение, пик в систолу, ремонт выбросов, бордер-путь, сглаживание метрик, видимость стенки, круговой проход. **Осталось:** точность на низком SNR (многошкальное совмещение, §5.3 п.5); авто-черновик контура КД (§5.3 п.3) — обводка остаётся главным остаточным источником смещения. |
| 2b. Фантом и валидация | **сделано** | `tests/fixtures/ste_phantom.py` — точная кинематика + имитация изображения (спеклы, декорреляция, out-of-plane, шум 0–40 дБ); `bench/ste_phantom.py` — 20 вариантов, KPI-ворота; ворота: 16 провалов KPI, все на строках `invalid` кроме `zero_snr20` (`review`, segment RMS 2.51 против порога 2.5), **на строках `valid` провалов нет**; GLS/ TTP на клинически значимых строках не сдвинулись (uniform 20 дБ −19.53 при истине −19.90, long_axis −19.52, gradient −10.53). Числа воспроизводятся буквально на `numpy==1.26.4` (`docs/STE_TRACKING_VERIFICATION.md` §3, §5). **Осталось:** прогон на реальных клипах — нужны gold-контуры КД (§11.2 п.1). |
| 3. QC + модель + протокол | **сделано** | `domain/models/ste_analysis.py`: `StrainAnalysis` (определения, якоря, QC, значения, кривые, JSON-схема v2, null вместо NaN) и `StrainStudy` (per-view GLS, `GLS_AV` только по валидным видам, слияние 18 сегментов с источниками, список неизмеренных); ворота Q4 (≤1 исключённого сегмента на вид), круговой проход Q6 (`closure_*`, ворота 0.10/0.25/0.30), видимость стенки Q4b (`qc_visibility_*`), объявления сравнимости (протяжённость ROI, компенсация трансляции, фактически применённое окно сглаживания, частота кадров, стороны стенок); экспорт JSON/CSV с блоком провенанса; метки недостоверного слежения ✕/○ на изображении; статус вида ●/⚠/■ в таблице результатов. **GLS в протоколе — сделано 2026-09-11** (§5.3 п.6): `StrainReport` как отчётная проекция study, хранение в `study_measurement_session` на исследование, секция в тексте отчёта и в PDF с QC-метками по видам, паритет чисел UI↔отчёт под тестом. **Фаза 3 закрыта полностью.** |
| 4. UI-паритет | **частично** | Сделано: 18-секторная мишень в стандартной раскладке с числами, палитра GE/EchoPAC (ярко-красный = норма), карта TTP, «нет данных» вместо выдуманных секторов; сводная таблица как у вендора (GLS, ESS, TTP, PSI, дрейф, per-view A4C/A2C/DAO, `GLS_AV`, ФВ/КДО/КСО, АвтоЗАК, ЧСС) + метка статуса вида; настройки: окно сглаживания кривых 1–31 кадр, зеркальное отображение, толщина стенки, компенсации; переглаживание пост-фактум слайдером; QC-чекбоксы сегментов «принять/отклонить» (как инструкция вендора) с пересчётом GLS; экспорт CSV/JSON/PNG. **Подписи сегментов у стенок на cine — сделано 2026-09-11** (`presentation/segment_labels.py`, `SpeckleOverlay.show_segment_labels`, тот же якорь и имена в `CinePanel`; QC-список — на полных AHA-именах; §6.5). **Осталось:** обзорный экран трёх видов в стиле «3 Point Contour» (§5.3 п.2), полоса деформации, векторы смещений, hover/click-связка мишень↔кривые↔cine, colorbar, ECG/transport v2, вкладка отчёта (§5.3 п.9). |
| 5. 3 вида | **частично** | Многосмотровая модель (`StrainStudy`), per-view GLS и `GLS_AV` в таблице и мета-строке, вид и статус в провенансе результата. **Осталось:** очередь анализа трёх видов из UI, кэш результатов на клип, headless CLI (§5.3 п.8). |
...
`src/echo_personal_tool/ui/strain_window.py`, `src/echo_personal_tool/presentation/{speckle_overlay,speckle_settings_dialog,ste_sensitivity_overlay}.py`,
тесты `tests/unit/test_ste_{quality,segment_map,strain_metrics,study_analysis,qc_honesty,phantom,real_clips,tracking_verification}.py`,
`tests/unit/test_{strain_node_curves,wall_visibility,strain_computation,strain_window,ui_strain}.py`,
бенчмарки `bench/ste_{phantom,trust_flags,contour_tracking}.py`,
документы `docs/STE_TRACKING_VERIFICATION.md`, `docs/STE_VENDOR_REFERENCE.md`.



### 5.3 Осталось сделать (очередь на следующую итерацию)

Приоритеты: P0 — блокирует заявление о вендорском уровне, P1 — клиническая полнота, P2 — усиление,
P3 — расширения. Оценки — рабочие дни одного разработчика, без учёта ожидания данных.
...
| 4 | **Подписи сегментов/стенок на cine** (P1) — **сделано** | Короткие имена вендорского стиля у стенок (`segment_labels.py` + `SpeckleOverlay.show_segment_labels`), якорь `segment_label_anchors` — снаружи стенки; тот же модуль в `CinePanel`; имена на все 18 сегментов в ru/en (заполнитель `strain.segment_name.{id}` заменён); `clear()`/кадр вне окна трекинга убирают подписи; тесты: `test_segment_labels.py` (8), якоря/подписи в `test_presentation_speckle_overlay.py` (+10), `test_strain_window.py` (2), сквозной `test_viewer_widget.py`; иллюстрация `docs/screenshots/ste-segment-labels.png` | `presentation/{segment_labels,speckle_overlay}.py`, `ui/strain_window.py` | 1 (факт) |
...
| 6 | **GLS в протокол исследования** (P1) — **сделано 2026-09-11** | Секция «Продольная деформация (STE)» в тексте отчёта и в PDF: GLS по видам с QC-меткой ●/⚠/■, `GLS AV` только по валидным видам (иначе явная строка «ни один вид не прошёл контроль»), метод, источник АК по видам, число неизмеренных сегментов, объявление черновика контура. `StrainReport` + `strain_report_from_study()` (проекция `StrainStudy`), хранение в `StudyMeasurementSessionStore.set_strain()` на исследование, `MeasurementSnapshot.strain`, накопление видов в `AppController._record_strain_result()` (повтор вида заменяет его), `StrainWindow.set_study()` перенимает то же накопление. Тесты: `test_ste_protocol_report.py` (29), включая паритет «числа UI == числа отчёта» и PDF с глифами меток | `domain/models/{measurements,ste_analysis}.py`, `domain/services/measurement_report_formatter.py`, `application/{app_controller,study_measurement_session}.py`, `ui/strain_window.py` | 1 (факт) |
| 7 | **Редактор границ сегментов** (UX-бэклог; консенсусом не требуется) | Сдвиг границы мышью → пересчёт сегментных кривых и мишени **без** повторного трекинга; границы уже параметризованы (`basal_fraction`/`apical_fraction`) | `ui/strain_window.py`, `domain/services/segment_map.py` | 1–2 |
| 8 | **Headless CLI** (P2) | `python -m echo_personal_tool.ste_cli analyze --view A4C --json out.json` без Qt; пакетная валидация на клипах | новый `application/ste_cli.py` | 1–2 |
| 9 | **Остаток фазы 4 UI** (P2) | Полоса деформации по эндокарду, векторы смещений, hover/click-связка мишень↔кривые↔cine, colorbar, ECG/transport v2, вкладка отчёта | `ui/strain_window.py`, `presentation/*` | 5–8 |
| 10 | **Расширения** (P3) | LA/RV strain, multi-beat + доверительные интервалы, плотное поле, ML-трек | новые модули | отдельные милстоуны |

Плюс два решения от пользователя (§11.2), которые не блокируют разработку, но блокируют сверку и merge:
**gold-контуры КД** для 2–3 клипов For_pero и **политика хранения фикстур** (кадры содержат впаянные ФИО).
...
```bash
python3 -m venv .venv
./.venv/bin/pip install -e ".[dev]" "numpy==1.26.4" "pylibjpeg-libjpeg==2.2.0" "pylibjpeg-openjpeg==2.2.1"
./.venv/bin/python tools/qtstub/mkstub.py          # заглушки Qt-библиотек для headless-прогонов
export LD_LIBRARY_PATH=tools/qtstub/lib QT_QPA_PLATFORM=offscreen
```

Проверки перед каждым коммитом (CI прогоняет и линт, и **формат** — без второго шага lint красный):
...
Список STE-файлов (30; +`test_segment_labels.py` — имена сегментов для cine/панели,
+`test_bench_render_utils.py` — страж ориентации иллюстраций, +`test_ste_protocol_report.py` — GLS
в протоколе и паритет чисел UI↔отчёт): `test_ste_{phantom,segment_map,qc_honesty,study_analysis,real_clips,quality,reproducibility,single_source,strain_metrics}.py`,
...
* `bench/ste_phantom.py` — 20 вариантов фантома, KPI-ворота; числа воспроизводимы буквально на `numpy==1.26.4` (§3 `docs/STE_TRACKING_VERIFICATION.md`);
...
этот план (§5.2 — состояние, §5.3 — очередь).

Данные: `tests/fixtures/for_pero` — 19 клипов (выгружаются CI-воркфлоу из закрытого репозитория `areatu/Sonoforge_data`, путь `data/dicom/For_pero`;
исходные клипы в песочнице недоступны, поэтому фикстуры — единственный источник кадров);
gold-контуры — только `gold105.dcm`; вендорские экраны лежат в тех же фикстурах (впаянные ФИО —
см. §11.2 п.2).
...
| Включение слоёв | `K` — ядра, `B` — полоса, `V` — векторы, `G` — подписи сегментов (✅ подписи реализованы, 2026-09-11: `presentation/segment_labels.py` + `SpeckleOverlay.show_segment_labels`, короткие имена у стенок на cine и в strain-окне; клавиша `G` пока не заведена) |
...
* **Подписи сегментов у стенок на cine** (по вендору: «БазПерг/СрБок» как «Inf. septum base» у GE)
  ✅ **Реализовано** (2026-09-11): короткие имена из `presentation/segment_labels.py` рисуются
  `SpeckleOverlay.show_segment_labels` **между кадром и зрителем** — якорь вычисляет
  `segment_label_anchors` (среднее узлов сегмента, сдвинутое от центра полости на `offset_px`), так что
  имя стоит снаружи стенки, а не поверх границы, которую читает врач. Внутри strain-окна тот же модуль
  и тот же якорь (`CinePanel.show_segment_labels`), поэтому сегмент читается одинаково на cine и в
  панели. Список QC-чекбоксов переведён на полные AHA-имена (`full_segment_label`) — там место есть.
  Подписи живут вместе с оверлеем: `clear()` и кадр вне окна трекинга их убирают.

  ![Подписи сегментов на cine](screenshots/ste-segment-labels.png)

  <sub>Рендер `bench/ste_segment_labels_preview.py`: кадр фантома (`tests/fixtures/ste_phantom.py`),
  позиции узлов — его точная кинематика, сегменты — рабочий `assign_segments_from_arc` (не
  третьи-сторонний скриншот и не ручная расстановка подписей).</sub>
...
* **Числа получены** (2026-09-11): `docs/STE_VENDOR_REFERENCE.md` — GLS по видам, снятые с экранов самих
  аппаратов для тех же исследований, что лежат в клипах: Samsung RS85 −19.0/−20.0/−14.5 → сред. −17.8 %
  (ФВ 48.3 %; на других датах −10.7 % и −8.0 %), Philips EPIQ −18.0/−25.0/−24.3 → −22.5 % и −20.7 %,
  GE/EchoPAC −17.4 %. Конвенции вендоров подтверждены: итоговое число — арифметическое среднее трёх видов,
  отчётное — пик, разброс между видами 1.4–8.6 п.п. (ориентир для нашего межвидового контроля).
* `IM_0059` (Philips, 46 кадров, 0.449 мм/px, ECG) — локальный клип автора для ручной проверки
  (в песочницу не передан; его роль выполняют клипы For_pero).
* `US003900.dcm` / `US006700.dcm` (Samsung) — сравнение вида мишени/кривых, цветовых решений.
* **Чего не хватает для количественной сверки:** gold-контуров КД для клипов For_pero — сейчас разметка
  есть только у `gold105.dcm`, поэтому наш GLS нельзя посчитать на тех же кадрах, где вендор дал число.
  См. §11.2 п.1.
...
| F12 | **Доверие к слежению ничем не подтверждалось**: в отчёте были NCC и «покрытие», а круговой проход (вперёд-назад) считался только внутри `track_cine_bidirectional`, только на сырых совпадениях и только как внутренний флаг. Клиницист не мог отличить «совпало по NCC» от «положение подтверждено», а автоматический трекинг контуров между кадрами выглядел самообъявленным. | новый проход `verify_trajectory_closure()` по **итоговой** траектории (интерполяция + ремонт выбросов + сглаживание) назад к КД; единый гейт `verification_gate_px()` = 0.5 × max(search_radius, 24); в отчёт — `qc_closure_median_mm` / `qc_closure_p95_mm` / `qc_rejected_fraction` / `qc_unverified_fraction`, ворота 0.10/0.25/0.30, ключ `strain.qc.reason.tracking_verification` (ru+en) и строка в meta strain-окна. Замеры: отклонено 3.4–6.1 % кадров-узлов на 20–40 дБ и 65.6–80.6 % на 10 дБ; ошибка контура p95 1.72 мм (40 дБ) / 3.38 мм (20 дБ); флаг на 10 дБ precision/recall 0.95/0.69 (>2 мм). **Ограничение:** проверка использует нарисованный контур КД как эталон и потому слепа к ошибке самой обводки (сдвиг +4 мм → +8.1 п.п. при `valid`) — эти два источника ошибки должны докладываться раздельно. | **сделано** 2026-09-11 |
...
измерение. Улучшение самих SNR — отдельная задача §5.3 п.5 (многошкальное совмещение). Дальше — фаза 4 (UI)
и очередь §5.3; текущие числа фантома и границы применимости — `docs/STE_TRACKING_VERIFICATION.md` §3–§5.
...
**Остаток по §5.3 (на 2026-09-11):** 19–34 дня, из них P0/P1 (сверка с вендором, обзорный экран трёх видов,
авто-черновик контура КД, подписи сегментов, GLS в протокол) — 10–19 дней; п.10 (расширения) не входит.
...
| 3 | Эталоны | **Числа получены** (экраны аппаратов, `docs/STE_VENDOR_REFERENCE.md`) и **клипы в репозитории** (`data/dicom/For_pero` + CI-выгрузка в `tests/fixtures/for_pero`). Референсные интерфейсы: Samsung «3 Point Contour», Philips AutoStrain, GE EchoPAC AFI (приложение F + §6.2). Google Drive из песочницы недоступен и больше не нужен. Для сверки на одних кадрах нужны gold-контуры КД — §11.2 п.1. |
...
### 11.2 Что нужно от пользователя (блокирует сверку и merge, но не разработку)

1. **Gold-контуры КД (эндокард) для 2–3 клипов For_pero**: точки + номер кадра + вид (в любом виде —
   JSON, CSV, DICOM-контуры, скриншот с замером). Сейчас разметка есть только у `gold105.dcm`, поэтому
   сверка нашего GLS с вендорским **на одних и тех же кадрах** невозможна, а сравнивать разные клипы
   бессмысленно (разброс между видами и аппаратами больше искомой разницы).
   *Альтернатива, если разметки нет:* разрешить построить контур КД полуавтоматически и сравнить с явной
   пометкой «контур не gold» — это даст первые числа, но не валидацию (см. §5.3 п.1 и п.3).
2. **Политика хранения фикстур**: кадры в `tests/fixtures/for_pero` содержат **впаянные ФИО**. Варианты:
   (а) маскировать зону имени в экспортёре и держать бандл в main; (б) не коммитить бандл в main, отдавать
   его артефактом CI (тесты без него скипаются, `test_ste_real_clips.py` это уже умеет). До решения PR в
   main не мержим.
3. *(опционально)* Ролики/скриншоты STE-интерфейса Samsung/Philips — пригодятся для обзорного экрана трёх
   видов (§5.3 п.2). Уже имеющихся 6 стоп-кадров хватает для сверки чисел и компоновки; `drive.google.com`
   из песочницы недоступен, файлы можно приложить в чат Arena или положить в репозиторий/Release.
...
```
ε_i(t)      = (L_i(t) − L_i(0)) / L_i(0) · 100             # % (Lagrange), L_i — 3-узловая поддуга
ε_seg(t)    = Σ_i w_i·ε_i(t) / Σ_i w_i                     # один кадр, w — доля длины сегмента в ED
ε_gl^A(t)   = (L_total(t) − L_total(0)) / L_total(0) · 100 # по всей открытой дуге (основное)
ε_gl^B(t)   = Σ_seg w_seg·ε_seg(t) / Σ_seg w_seg           # эквивалентный способ (контроль)
GLS_view    = min{ ε_gl^A(t) : t ∈ [ED … ED+0.55·RR] }
ESS_view    = ε_gl^A(AVC)
GLS_AV      = mean(GLS_A4C, GLS_A2C, GLS_A3C)
...
### C. Чек-лист приёмки (с отметками на 2026-09-11)

- [x] 0 узлов/ядер вдоль хорды МК; трекинг идёт по открытой дуге — `resample_open_arc`, тесты `test_ste_segment_map.py`.
- [x] Сегменты по дуге ED; покрытие 100 % узлов; континуальность; сторона стенки определяется изображением
      (разворот обводки не переименовывает сегменты), зеркальное отображение выбирается и объявляется.
      *Осталось:* подписи сегментов/уровней на cine (п.4 §5.3).
- [x] Одно определение strain в worker, UI и экспорте (тест равенства чисел) — `lagrangian_strain_pct`,
      `test_ste_single_source.py`.
- [x] ED/ES/AVC: источники видны и правимы; L0 всегда из ED (`ed_es_source`, `avc_source`, ручные ED/ES).
- [x] GLS = пик глобальной кривой; ESS и пиковый — раздельные параметры с подписями (`DEFINITION_*`, CSV/JSON).
- [x] QC-статус невозможен без геометрии/покрытия/самосогласованности/физиологичности (+ видимость стенки Q4b,
      круговой проход Q6, шум против сокращения F6).
- [x] Фантом: GLS ±1 п.п. на клинически значимых строках; инварианты жёсткого движения и нулевой деформации —
      `bench/ste_phantom.py`, тесты `test_ste_phantom.py` (§7.5).
- [x] Мишень: только посчитанные сегменты; «нет данных» заштриховано; TTP-мишень; метка статуса вида в таблице.
- [ ] UI: Focus/Quad/Report, кривые, таблица, ECG-маркеры, правка landmarks, undo/redo, экспорт.
      *Сделано:* кривые модели, сводная таблица, ECG-маркеры, QC-правка сегментов, экспорт CSV/JSON/PNG.
      *Осталось:* Focus/Quad/Report и обзорный экран трёх видов, undo/redo, правка landmarks (§5.3 п.2, п.9).
- [ ] Результат в сессии измерений и в отчёте — *осталось* (§5.3 п.6); ru+en ключи — сделано.
- [x] Детерминизм (фиксированный порядок обхода, `config_hash` в провенансе) и локальный набор тестов зелёный.
      *Осталось:* бюджеты времени §8 и headless CLI для пакетных прогонов (§5.3 п.8).
