...
> **Реальные данные в репозитории (с 2026-09-11):** `data/dicom/For_pero` (19 клипов, Git LFS, видны только на
> default branch) → CI-воркфлоу `.github/workflows/ste-fixtures.yml` выгружает компактную производную в
...
* **Фаза 4 (частично)**: мишень 18 сегментов в раскладке GE с палитрой GE/EchoPAC, карта TTP, «нет данных»
  ...
  CSV/JSON/PNG.
* **Реальные данные и вендорские эталоны**: `data/dicom/For_pero` (19 клипов, LFS) → CI-воркфлоу
...
Данные: `tests/fixtures/for_pero` — 19 клипов (выгружаются CI-воркфлоу из `data/dicom/For_pero`;
LFS-контент в песочнице недоступен, поэтому фикстуры — единственный источник кадров);
