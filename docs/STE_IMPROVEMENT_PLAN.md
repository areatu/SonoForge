...
**Состояние на 2026-09-11** (`tests/unit/test_ste_phantom.py`, `bench/reports/ste_phantom_quick.json`):
инварианты модели (1-й уровень — сама кинематика, round-trip карты, нулевая деформация при жёстком
движении, точность однородного режима, физиологичность профиля, детерминизм кадров) выполняются
**точно**; инварианты пайплайна — частично: жёсткая трансляция 15 px даёт |GLS| 0.33 п.п. (≤0.5 ✓),
чистая ротация — 0.55 п.п. (на границе), большая жёсткая ротация+трансляция — **+26.8 п.п. (дефект)**,
шум 5–10 дБ — **+15…+31 п.п. (дефект)**, QC при этом умеет сообщить `valid` (дефект). Перечислено в §7.5.
...
| F2 | **Стационарный кламп стенки срезает деформацию**: `clamp_trajectories_to_wall(..., inward_slack=1.4, outward_slack=0.0)` держит ядра в полосе, построенной по ED-геометрии, а конец в систолу уходит внутрь дальше, чем 1.4 толщины стенки. При этом 41 % кадро-ядер всё равно оказываются вне истинной стенки (кламп решает не ту задачу) | однородный фантом, истина −19.9 п.п.: с клампом −14.30 (bias +5.60), без клампа −18.50 (+1.40); сегментный MAE 6.57 → 5.47 п.п. | **открыто** |
| F3 | **Жёсткое движение превращается в деформацию**: трансляция 30 px или ротация 25° + сдвиг 25 px дают +12.6 и +26.8 п.п. при истине 0.0 (смещение выходит за окно поиска, совпадение прижимается к краю) | таблица §7.2, тест `test_worker_ignores_large_rigid_probe_motion` (strict xfail) | **открыто** |
| F4 | **QC разрешает неверный результат**: при 10 дБ + декорреляции 0.25 пайплайн сообщает `qc_status='valid'` при ошибке GLS 15.3 п.п. (принято 100 % ядер) — ровно жалоба «качество >95 %, GLS неверен» | тест `test_quality_never_hides_a_large_strain_error` (strict xfail) | **открыто** |
...
| F6 | **Сегментные значения расходятся сильнее глобального**: RMS 9.0 п.п. (20 дБ), 15.6 п.п. (10 дБ), 35.9 п.п. (0 дБ) при воротах 2.5 п.п. | `segment_rms_pp` в отчёте бенчмарка | **открыто**, следствие F2-F3 |

Вывод: до ворот §3.7 пайплайн не дотягивает, причём основной вклад дают F2 и F3 (геометрия полосы и
обработка жёсткого движения), а не NCC. Следующий шаг после фантома — закрыть F2/F3/F4 в пределах STE-модуля
и затянуть ворота KPI в тестах до 1.0 п.п. (сейчас тест фиксирует текущее состояние 7.0 п.п. и подлежит
ужесточению).
