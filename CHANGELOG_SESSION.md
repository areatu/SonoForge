...
## [2026-09-05] Splash v4 / v4.1: без брызг, крупные Bold-слова асимметрично, крупные проценты; вариант 4.1 — компактная карточка
- **Тип:** feature (изменение дизайна по уточнению пользователя)
- **Файлы:** `src/echo_personal_tool/presentation/splash.py`, `src/echo_personal_tool/main.py` (режим `ECHO_SPLASH_COMPACT=1`), `tests/unit/test_splash_screen.py`, `docs/splash-preview/{index.html,index4-1.html,index3.html,index2.html,index1.html,README.md}`
- **Суть:** Дочистка концепта logo-fill по замечаниям: (1) брызги/капли удалены полностью — единственная «грязь» осталась в виде синусоидального края самой маски (`_LogoFill` рисует только волну: 2,6 периода, амплитуда 2,4 px при эталонной ширине); (2) слова «Local-first · Private · ASE aligned» теперь заметно крупнее и жирные — `_WORD_PT_REF=20` pt при `_LOGO_W_REF=340` px, `QFont.DemiBold` (было 14 pt/Normal), blur 16 px → 0, зоны 16–58/34–76/52–96 %; размещены асимметрично (`_relayout`): слово 0 — левее-выше логотипа близко, слово 1 — правее-выше выше и дальше, слово 2 — левее-ниже далеко вниз; (3) счётчик процентов укрупнён — `_PCT_PT_REF=22` pt DemiBold (было 12 pt); (4) опциональный плоский фон `SplashScreen(background=<path>)` — cover-fit картинка (например, 4-камерное эхо) на 20 % непрозрачности, по умолчанию выключен; (5) новый вариант **4.1** `SplashScreen(compact=True)`: вместо fullscreen — центрированная карточка 620×430, радиус 26 px, масштабируется на малых экранах (множитель ≥0,55), после финального кадра уходит в главное окно. В `main.py` вариант 4.1 включается env `ECHO_SPLASH_COMPACT=1` (для сравнения без пересборки). Превью: `docs/splash-preview/index.html` (вариант 4, fullscreen) и `index4-1.html` (4.1, карточка); вариант 3 с брызгами сохранён архивом `index3.html`. Исправлен найденный при ревью баг v3: `_Word` создавался без родителя (слова не попали бы на экран) — теперь слова создаются с `parent=self`. Проверено: ruff clean, py_compile.
...
## [2026-08-18] Fix: frame-skip jumps on large RGB cines (gold7/gold11/gold12)
- **Тип:** fix
- **Файлы:** `src/echo_personal_tool/application/frame_cache.py`, `tests/unit/test_frame_cache_extended.py`
- **Суть:** Устранены перескакивания кадров (прыжок 7-8 → 70-73 → конец, зацикливание) на больших RGB-цинах (79-86 кадров × ~3 МБ = 230-250 МБ при кэше 64 МБ). Причины и исправления: (1) `_evict_to_memory_limit()` теперь сохраняет непрерывную дугу вперёд от playhead (`_forward_arc`), а не круговую дистанцию — прежний порядок вытеснения держал wrap-tail как «близкий» и выбивал середину, создавая дыры; (2) `loaded_ahead()` теперь считает только НЕПРЕРЫВНЫЙ загруженный хвост от center+1 (с заворотом), а не все загруженные кадры дальше по индексу — иначе prefetch `start=center+1+ahead` перескакивал через дыру; (3) `_evict()` дополнительно сохраняет wrap-head кадры (0..wrap_head) у конца цины. Keep-all guard для малых цин (gold2, 55 кадров) сохранён без изменений. Проверено: симуляция playback-петли AppController + FrameCache на параметрах gold11/gold2 — 0 skip-ahead, 0 прыжков, 6-15 чистых циклов при decode latency 60/150/300 мс; 141 unit-тест и ruff — зелёные.

## [2026-08-21] Fix: Doppler manual calibration after auto-cal + time scale preservation
- **Тип:** fix
- **Файлы:** `src/echo_personal_tool/presentation/viewer_widget.py`, `src/echo_personal_tool/presentation/main_window.py`, `tests/unit/test_viewer_widget.py`
- **Суть:** (1) Возвращён 2-click мастер ручной velocity-калибровки — базовая линия снова является первой точкой скорости (`_begin_doppler_velocity_calibration(start_y=...)`), диалог шкалы скоростей открывается после ОДНОГО клика по скорости; регрессия внесена в commit 3e55bac, который заменил start_y на None. (2) Сброс калибровки больше не стирает временную шкалу: `clear_doppler_calibration_display(keep_time_scale=True)` сохраняет time_span/origin/from_dicom в time-only state, который подхватывает `_prompt_spectral_velocity_span`. (3) Бейзлайн-клик не обнуляет time axis на дисплее во время мастера. Детекторы обёрнуты в try/except, чтобы не блокировать мастер. Добавлены regression-тесты; удалены временные test_repro_*.py.

## [2026-09-06 15:46] Doppler: evidence-fusion baseline + Samsung luminance tick retry
- **Тип:** feature + fix
- **Файлы:** `src/echo_personal_tool/domain/services/doppler_baseline_evidence.py`, `src/echo_personal_tool/infrastructure/vendor_calibration_bridge.py`, `src/echo_personal_tool/infrastructure/samsung_tick_detector.py`
- **Суть:** (1) Cherry-pick `757a160` (ветка arena/01a06c36-sonoforge): evidence-fusion baseline-детектор (cue_line + cue_envelope_foot, conf≥0.5) встроен перед plateau-эвристикой в Samsung-tick путь (3 точки вызова; plateau — fallback). Исправлены baseline: US005000 493→394, 14.dcm 843→394 (подтверждено анализом строк: единственная нарисованная линия std=0 на y=392-393); US004200/US003700 без изменений (523/823 — верны). (2) Фикс потери ROI на US004200 в аппаратном потоке: luminance-конвертация RGB «вымывает» цветные тики, ложная периодика (spacing 187.5px, y=823) обгоняла реальную линейку (60px, y=874) и отбрасывалась spacing-gate'ом. Добавлен retry с brightness_threshold=90 при непрохождении sanity-gates (detect_ticks/_tick_score параметризованы). US004200: None → ROI (0,473)-(1179,871), base=523, t=3665ms. Регресс: 36 файлов трёх вендоров — изменился только US004200. Ветка test/doppler-evidence готова к merge в main.
